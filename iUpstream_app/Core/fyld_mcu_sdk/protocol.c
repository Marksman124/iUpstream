...
unsigned long Wifi_OTA_Pack_Len=0;					// 总长度
...
const DOWNLOAD_CMD_S download_cmd[] =
{
    ...
    {DPID_SET_TRAIN_PLAN_01, DP_TYPE_RAW},
    {DPID_SET_TRAIN_PLAN_02, DP_TYPE_RAW},
...
void all_data_update(void)
{
    ...
    mcu_dp_raw_update(DPID_SET_TRAIN_PLAN_01,当前训练计划01指针,当前训练计划01数据长度); //RAW型数据上报;
    mcu_dp_raw_update(DPID_SET_TRAIN_PLAN_02,当前训练计划02指针,当前训练计划02数据长度); //RAW型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_system_working_status_handle
功能描述 : 针对DPID_SYSTEM_WORKING_STATUS的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
/*****************************************************************************
函数名称 : dp_download_motor_current_speed_handle
功能描述 : 针对DPID_MOTOR_CURRENT_SPEED的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_motor_current_speed_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long motor_current_speed;

    motor_current_speed = mcu_get_dp_download_value(value,length);
    /*
...
/*****************************************************************************
函数名称 : dp_download_motor_current_time_handle
功能描述 : 针对DPID_MOTOR_CURRENT_TIME的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_motor_current_time_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long motor_current_time;

    motor_current_time = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    */
		if(If_Accept_External_Control())
...
/*****************************************************************************
函数名称 : dp_download_free_mode_speen_handle
功能描述 : 针对DPID_FREE_MODE_SPEEN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_free_mode_speen_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long free_mode_speen;

    free_mode_speen = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_FREE_MODE_SPEEN,free_mode_speen);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_timing_mode_speen_handle
功能描述 : 针对DPID_TIMING_MODE_SPEEN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_timing_mode_speen_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long timing_mode_speen;

    timing_mode_speen = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_TIMING_MODE_SPEEN,timing_mode_speen);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_timing_mode_time_handle
功能描述 : 针对DPID_TIMING_MODE_TIME的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_timing_mode_time_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long timing_mode_time;

    timing_mode_time = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_TIMING_MODE_TIME,timing_mode_time);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_set_train_plan_01_handle
功能描述 : 针对DPID_SET_TRAIN_PLAN_01的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_set_train_plan_01_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
...
/*****************************************************************************
函数名称 : dp_download_set_train_plan_02_handle
功能描述 : 针对DPID_SET_TRAIN_PLAN_02的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_set_train_plan_02_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
...
/*****************************************************************************
函数名称 : dp_download_set_train_plan_03_handle
功能描述 : 针对DPID_SET_TRAIN_PLAN_03的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_set_train_plan_03_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
...
/*****************************************************************************
函数名称 : dp_download_set_train_plan_04_handle
功能描述 : 针对DPID_SET_TRAIN_PLAN_04的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_set_train_plan_04_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
...
/*****************************************************************************
函数名称 : dp_download_set_train_plan_05_handle
功能描述 : 针对DPID_SET_TRAIN_PLAN_05的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_set_train_plan_05_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
        case DPID_SYSTEM_WORKING_STATUS:
            //系统状态机处理函数
            ret = dp_download_system_working_status_handle(value,length);
        break;
        case DPID_MOTOR_CURRENT_SPEED:
            //当前转速处理函数
            ret = dp_download_motor_current_speed_handle(value,length);
        break;
        case DPID_MOTOR_CURRENT_TIME:
            //当前时间处理函数
            ret = dp_download_motor_current_time_handle(value,length);
        break;
        case DPID_FREE_MODE_SPEEN:
            //自由模式默认速度处理函数
            ret = dp_download_free_mode_speen_handle(value,length);
        break;
        case DPID_TIMING_MODE_SPEEN:
            //定时模式默认速度处理函数
            ret = dp_download_timing_mode_speen_handle(value,length);
        break;
        case DPID_TIMING_MODE_TIME:
            //定时模式默认时间处理函数
            ret = dp_download_timing_mode_time_handle(value,length);
        break;
        case DPID_SET_TRAIN_PLAN_01:
            //训练计划01处理函数
            ret = dp_download_set_train_plan_01_handle(value,length);
        break;
        case DPID_SET_TRAIN_PLAN_02:
            //训练计划02处理函数
            ret = dp_download_set_train_plan_02_handle(value,length);
        break;
        case DPID_SET_TRAIN_PLAN_03:
            //训练计划03处理函数
            ret = dp_download_set_train_plan_03_handle(value,length);
        break;
        case DPID_SET_TRAIN_PLAN_04:
            //训练计划04处理函数
            ret = dp_download_set_train_plan_04_handle(value,length);
        break;
        case DPID_SET_TRAIN_PLAN_05:
            //训练计划05处理函数
            ret = dp_download_set_train_plan_05_handle(value,length);
        break;
...
unsigned char mcu_firm_update_handle(const unsigned char value[],unsigned long position,unsigned short length)
{
    ...
    if(length == 0) {
      ...
      if(position == REG_FILE_NUMBER_STAR) // 起始包
			...
			taskENTER_CRITICAL();
			write_addr = (FLASH_APP_PATCH_ADDR + Wifi_OTA_Pack_Len);
			iap_write_appbin(write_addr,(unsigned char*)value,length/2);
