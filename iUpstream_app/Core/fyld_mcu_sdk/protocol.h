...
/******************************************************************************
                       1:修改产品信息
******************************************************************************/
#define PRODUCT_KEY "iupstream1"    //开发平台创建产品后生成的16位字符产品唯一标识

#define MCU_VER 	MACRO_SOFTWARE_VERSION_UINT32         //用户的软件版本,用于MCU固件升级,MCU升级版本需修改。
                                //多MCU格式为“1.0.0,1.0.1,1.0.2,...”，以英文逗号做分隔符，第一个逗号前面固件的通道号固定为9，其他按顺序依次递增10,11,...,编号必须连续且与平台的一致
...
#else
#define WIFI_UART_RECV_BUF_LMT          WIFI_DATA_PROCESS_LMT             //串口数据接收缓存区大小,如MCU的RAM不够,可缩小
...
//MOS温度(只上报)
//备注:
#define DPID_GET_MOS_TEMPERATURE 6
//电箱温度(只上报)
//备注:
...
//系统状态机(可下发可上报)
//备注:
#define DPID_SYSTEM_WORKING_STATUS 22
//当前转速(可下发可上报)
//备注:
#define DPID_MOTOR_CURRENT_SPEED 23
//当前时间(可下发可上报)
//备注:
...
//定时模式默认速度(可下发可上报)
//备注:
#define DPID_TIMING_MODE_SPEEN 29
//定时模式默认时间(可下发可上报)
//备注:
...
//训练计划02(可下发可上报)
//备注:训练计划包含50段速，每段包含【uint16-速度（百分比）】+【uint16 - 时间（秒）】，总长度【2+2】+50 = 200 字节
#define DPID_SET_TRAIN_PLAN_02 32
//训练计划03(可下发可上报)
//备注:训练计划包含50段速，每段包含【uint16-速度（百分比）】+【uint16 - 时间（秒）】，总长度【2+2】+50 = 200 字节
#define DPID_SET_TRAIN_PLAN_03 33
//训练计划04(可下发可上报)
//备注:训练计划包含50段速，每段包含【uint16-速度（百分比）】+【uint16 - 时间（秒）】，总长度【2+2】+50 = 200 字节
#define DPID_SET_TRAIN_PLAN_04 34
//训练计划05(可下发可上报)
//备注:训练计划包含50段速，每段包含【uint16-速度（百分比）】+【uint16 - 时间（秒）】，总长度【2+2】+50 = 200 字节
